...
def prepare_poms(release: str, platform: str, snapshot: bool = True) -> Tuple[
    str, str]:
    ...
    for pom in ["pom", "pom_all"]:
        ...
        with open(pom_file) as f:
            ...
            pom_content = re.sub(r"<version>.*</version>",
                                 f"<version>{release}-{SUB_VERSION}{p_suffix}</version>",
                                 pom_content, count=1)
            pom_content = pom_content.replace("${project.platform}", platform)
            pom_content = pom_content.replace("${project.artifactId}",
                                              f"ap-loader-{platform}")
            pom_content = re.sub(r"<project.vversion>.*</project.vversion>",
                                 f"<project.vversion>{release}</project.vversion>",
                                 pom_content, count=1)
            pom_content = re.sub(r"<project.subversion>.*</project.subversion>",
                                 f"<project.subversion>{SUB_VERSION}</project.subversion>",
                                 pom_content, count=1)
            pom_content = re.sub(r"<project.platform>.*</project.platform>",
                                 f"<project.platform>{platform}</project.platform>",
                                 pom_content, count=1)
            pom_content = re.sub(r"<project.suffix>.*</project.suffix>",
                                 f"<project.suffix>{p_suffix}</project.suffix>",
                                 pom_content, count=1)
...
class PreparedPOMs:
    ...
    def __enter__(self):
        self.pom, self.pom_all = prepare_poms(self.release, self.platform,
                                              self.snapshot)
        return self
...
def download_json(url, path: str) -> Any:
    ...
    if not os.path.exists(cache_path) or os.path.getmtime(
            cache_path) + CACHE_TIME <= time.time():
...
def get_releases(tool: Tool) -> List[Dict[str, Any]]:
    """ Return the releases of async-profiler from the GitHub API """
    if tool == Tool.ASYNC_PROFILER:
        return download_json(
            "https://api.github.com/repos/jvm-profiling-tools/async-profiler/releases",
            "releases.json")
    elif tool == Tool.JATTACH:
        return download_json(
            "https://api.github.com/repos/jattach/jattach/releases",
            "jattach_releases.json")
    else:
        raise Exception("Unknown tool: " + tool.name)


def get_release(tool: Tool, release: str) -> Dict[str, Any]:
    rel = [release_dict for release_dict in get_releases(tool) if
           release_dict["tag_name"] == "v" + release]
    if rel:
        return rel[0]
    print(
        f"Release {release} for {tool} not found, available releases are: {', '.join(get_release_versions(tool))}",
        file=sys.stderr)
    sys.exit(1)


def get_release_versions(tool: Tool) -> List[str]:
    return [release["tag_name"][1:] for release in get_releases(tool) if
            not release["tag_name"][1:].startswith("1.") and
            release["tag_name"][-1].isdigit()]
...
def get_release_info(tool: Tool, release: str) -> str:
    return get_release(tool, release)["body"]


def is_tar_gz(url: str) -> bool:
...
def is_compressed_file(url: str) -> bool:
    return is_tar_gz(url) or is_zip(url)


def get_release_asset_urls(tool: Tool, release: str) -> List[str]:
    return [url for url in (asset["browser_download_url"] for asset in
                            get_release(tool, release)["assets"]) if
            is_compressed_file(url)]


def release_platform_for_url(archive_url: str) -> str:
    if "jattach" in archive_url:
        return archive_url.split("jattach-")[-1].split(".")[-2]
    return re.split("[0-9][0-9.]+-", archive_url.split("/")[-1])[-1].split(".")[
        0]
...
def get_ending(url: str) -> str:
    return \
    [ending for ending in [".zip", ".tar.gz", ".tgz"] if url.endswith(ending)][
        0]
...
def download_latest_jattach(dest_path: Path, platform: str):
    ...
    url = ([url for url in urls if
            is_compressed_file(url) and release_platform_for_url(
                url) == platform] + [None])[0]
    if not url:
        raise Exception(f"Could not find jattach release for {platform}")
    ending = get_ending(url)
    archive_file = f"{CACHE_DIR}/jattach-{release}-{platform}{ending}"
    if not os.path.exists(archive_file):
        request.urlretrieve(url, archive_file)
    Path(dest_path).parent.mkdir(parents=True, exist_ok=True)
    execute(["tar", "-xzf", archive_file, "-C", dest_path.parent])
...
def download_release_url(release: str, release_url: str):
    ...
    Downloads the latest jattach release too if needed
    """
...
def download_release_code(release: str):
    ...
    if not os.path.exists(archive_file):
        request.urlretrieve(
            f"https://github.com/jvm-profiling-tools/async-profiler/archive/refs/tags/v{release}.zip",
            archive_file)
    execute(["unzip", "-o", archive_file, "-d", f"{CURRENT_DIR}/ap-releases"])
    shutil.rmtree(dest_folder, ignore_errors=True)
    shutil.move(f"{CURRENT_DIR}/ap-releases/async-profiler-{release}",
                dest_folder)
...
def build_release(release: str):
    ...
    for platform in get_release_platforms(Tool.ASYNC_PROFILER, release):
        ...
        execute(
            f"mvn -Duser.name='' -Dproject.vversion={release} -Dproject.subversion={SUB_VERSION} -Dproject.platform={platform} package assembly:single")
    ...
    execute(
        f"mvn -Duser.name='' -Dproject.vversion={release} -Dproject.subversion={SUB_VERSION} -Dproject.platform=all package assembly:single -f pom_all.xml")
    shutil.copy(
        f"{CURRENT_DIR}/target/ap-loader-all-{release}-{SUB_VERSION}-full.jar",
        all_target)
...
def build_tests(release: str):
    ...
    subprocess.check_call(
        f"java -jar {release_target_file(release, 'all')} clear", shell=True)
...
def test_release_basic_execution(release: str, platform: str,
    ...
    try:
        ...
        agentpath = subprocess.check_output(agentpath_cmd, shell=True,
                                            stderr=subprocess.DEVNULL).decode().strip()
        if (not agentpath.endswith(".so") and not agentpath.endswith(
                ".dylib")) or not os.path.exists(agentpath):
...
def test_releases_basic_execution(release: str):
    ...
    results = [platform for platform in
               get_release_platforms(Tool.ASYNC_PROFILER, release) if
               test_release_basic_execution(release, platform)]
    if not results:
        for platform in get_release_platforms(Tool.ASYNC_PROFILER, release):
    ...
    if len(results) > 1:
        raise Exception(
            f"Multiple platform JARs work for {release}: {results}, this should not be the case")
...
def get_changelog(release: str) -> str:
    url = get_release(Tool.ASYNC_PROFILER, release)["html_url"]
    return f"## ap-loader v{SUB_VERSION}\n\n{RELEASE_NOTES}\n\n" \
           ...
           f"The source code linked below should be ignored._\n\n{get_release_info(Tool.ASYNC_PROFILER, release)}"


def deploy_github(release: str):
    changelog = get_changelog(release)
    is_latest = release == get_most_recent_release(Tool.ASYNC_PROFILER)
    title = f"Loader for {release} (v{SUB_VERSION}): {get_release(Tool.ASYNC_PROFILER, release)['name']}"
    prerelease = get_release(Tool.ASYNC_PROFILER, release)["prerelease"]
    print(f"Deploy {release}-{SUB_VERSION} ({title}) to GitHub")
    if not os.path.exists(
            f"{CURRENT_DIR}/releases/ap-loader-all-{release}-{SUB_VERSION}.jar"):
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for platform in get_release_platforms(Tool.ASYNC_PROFILER, release) + [
            ...
            shutil.copy(
                f"{releases_dir}/ap-loader-{platform}-{release}-{SUB_VERSION}.jar",
                path)
...
def parse_cli_args() -> Tuple[List[str], Optional[str]]:
    ...
    if not commands:
        print(HELP)
    if not release:
        release = get_most_recent_release(Tool.ASYNC_PROFILER)
